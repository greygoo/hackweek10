...
#include "/home/pi/hackweek_10/facerec-0.4/include/facerec.hpp"	//<-- to modify
...
static void default_status(RASPIVID_STATE *state)
{
   ...
   state->timeout 			= 0;     // capture time : here 65 s
...
////////////////////////////////
	...
	// printf("x_dead_min: %d, x_dead_max: %d\n", x_dead_min, x_dead_max);

	// detect faces
	face_cascade.detectMultiScale(gray, faces, 1.1, 3, CV_HAAR_SCALE_IMAGE, Size(60,60));
	if(faces.size() > 0)
	{
		// get rectangle around face
		Rect face_i = faces[0];
		face = gray(face_i);
		...
		//printf("face center: %d, %d  face height / width: %d %d \n", pos_x, pos_y, face_height, face_width);
		...
		{ 
			printf("position y(%d) is below y_dead_min:(%d)\n", pos_y, y_dead_min);
		...
		{
			printf("position y(%d) is above y_dead_max(%d)\n", pos_y, y_dead_max);
...
/////////////////////////////////
	...
	cout<<"start\n";
	   if ((argc != 4)&&(argc!=3)) {
	       cout << "usage: " << argv[0] << " ext_files  treshold(opt) \n files.ext histo(0/1) 5000 \n" << endl;
	       exit(1);
	   }
	
	// set value by default for prediction treshold = minimum value to recognize
	if (argc==3) { trace("(init) prediction treeshold = 4500.0 by default");PREDICTION_TRESHOLD = 4500.0;}
	if (argc==4) PREDICTION_TRESHOLD = atoi(argv[3]);
	...
	// life is hard.
	people[P_DOMINIK] 	= "Dominik";
	people[P_CHRIS] 	= "Chris";
