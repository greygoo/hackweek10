...
////////////////////////////////
	...
	face_cascade.detectMultiScale(gray, faces, 1.1, 1, CV_HAAR_SCALE_IMAGE, Size(80,80));
	if(faces.size() > 0)
	{
		Rect face_i = faces[0];
		face = gray(face_i);
		...
		printf("x y coordinates: %d,%d  height: %d width: %d \n",pos_x,pos_y, face_height, face_width);
		int center_x = pos_x + ( face_width / 2 );
		int center_y = pos_y + ( face_height / 2 );
